...
<form id="filterForm" method="GET" action="{{ url('/invoices/vendorReport2') }}">
    <div class="label">
...
<form id="monthlyReportForm" method="GET" action="{{ url('/invoices/vendorReport') }}">
    <div class="label">
...
function fetchMonthlyData(revenueItemName, month) {
    fetch(`{{ url('/invoices/vendorReport') }}?revenue_item_name=${revenueItemName}&month=${month}`, {
        headers: {
...
function fetchSubheads(agencyName, detailsRow) {
    fetch(`{{ url('/invoices/vendorReport2') }}?agency_name=${agencyName}`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
        }
    })
...
function fetchPayments(subheadName, paymentsRow) {
    fetch(`{{ url('/invoices/vendorReport2') }}?subhead_name=${subheadName}`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
        }
    })
