...
<form id="filterForm" method="GET" action="{{ url('/invoices/report') }}">
    <div class="label">
...
<form id="monthlyReportForm" method="GET" action="{{ url('/invoices/report') }}">
    <div class="label">
    ...
    <select id="agency" name="agency_name">
        ...
        @foreach ($agenciesList as $agency)
            <option value="{{ \Illuminate\Support\Str::slug($agency) }}">{{ $agency }}</option>
        @endforeach
    </select>
    <div class="label">
    ...
    <!--    @for ($m = 1; $m <= 12; $m++)-->
    <!--        <option value="2025-{{ str_pad($m, 2, '0', STR_PAD_LEFT) }}">{{ date('F', mktime(0, 0, 0, $m, 10)) }}</option>-->
    <!--    @endfor-->
    <!--</select>-->
...
<script>
    document.getElementById('year').addEventListener('change', function() {
        ...
        for (let m = 1; m <= 12; m++) {
            let monthName = new Date(selectedYear, m - 1).toLocaleString('default', { month: 'long' });
            let paddedMonth = m.toString().padStart(2, '0');
            let option = new Option(monthName, `${selectedYear}-${paddedMonth}`);
...
<div style="margin-top: 20px; display: flex; gap: 20px;">
    ...
    <button onclick="downloadAgencyAsExcel()">Download as Excel</button>
...
function fetchMonthlyData(agencyName, month) {
    fetch(`{{ url('/invoices/report') }}?agency_name=${agencyName}&month=${month}`, {
        headers: {
...
function downloadAgencyAsExcel() {
    var table = document.getElementById('invoice');
    var wb = XLSX.utils.table_to_book(table, { sheet: "Sheet1" });
    XLSX.writeFile(wb, 'AgencyReport.xlsx');
}
