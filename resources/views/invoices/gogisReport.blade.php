...
<form id="filterForm" method="GET" action="{{ url('/invoices/gogisReport2') }}">
    <div class="label">
...
<form id="monthlyReportForm" method="GET" action="{{ url('/invoices/gogisReport') }}">
    <div class="label">
        <label for="agency">Agency:</label>
    </div>
    <select id="agency" name="agency_name">
        <option value="{{ \Illuminate\Support\Str::slug('GOMBE GEOGRAPHIC INFORMATION SYSTEM GOGIS') }}">GOMBE GEOGRAPHIC INFORMATION SYSTEM GOGIS</option>
    </select>
    <div class="label">
...
function fetchMonthlyData(agencyName, month) {
    fetch(`{{ url('/invoices/gogisReport') }}?agency_name=${agencyName}&month=${month}`, {
        headers: {
...
function fetchSubheads(agencyName, detailsRow) {
    fetch(`{{ url('/invoices/gogisReport2OGIS') }}?agency_name=${agencyName}`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
        }
    })
...
function fetchPayments(subheadName, paymentsRow) {
    fetch(`{{ url('/invoices/bureauReport2') }}?subhead_name=${subheadName}`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
        }
    })
