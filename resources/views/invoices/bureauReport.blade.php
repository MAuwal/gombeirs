...
<form id="filterForm" method="GET" action="{{ url('/invoices/bureauReport2') }}">
    <!--<div class="label">-->
...
<form id="monthlyReportForm" method="GET" action="{{ url('/invoices/bureauReport') }}">
    <div class="label">
...
function fetchMonthlyData(agencyName, month) {
    fetch(`{{ url('/invoices/bureauReport') }}?agency_name=${agencyName}&month=${month}`, {
        headers: {
...
function fetchSubheads(agencyName, detailsRow) {
    fetch(`{{ url('/invoices/bureauReport2') }}?agency_name=${agencyName}`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
        }
    })
