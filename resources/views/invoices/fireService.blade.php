...
<form id="filterForm" method="GET" action="{{ url('/invoices/fireServiceReport') }}">
    <div class="label">
...
<form id="monthlyReportForm" method="GET" action="{{ url('/invoices/fireServiceReport2') }}">
    <div class="label">
        <label for="agency">Agency:</label>
    </div>
    <select id="agency" name="agency_name">
        <option value="{{ \Illuminate\Support\Str::slug('PUBLIC PROCUREMENT BUREAU- DUE PROCESS') }}">PUBLIC PROCUREMENT BUREAU- DUE PROCESS</option>
    </select>
    <div class="label">
...
function fetchMonthlyData(agencyName, month) {
    fetch(`{{ url('/invoices/fireServiceReport2') }}?agency_name=${agencyName}&month=${month}`, {
        headers: {
...
function fetchSubheads(agencyName, detailsRow) {
    fetch(`{{ url('/invoices/fireServiceReport') }}?agency_name=${agencyName}`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
        }
    })
...
function fetchPayments(subheadName, paymentsRow) {
    fetch(`{{ url('/invoices/fireServiceReport') }}?subhead_name=${subheadName}`, {
        headers: {
            'X-Requested-With': 'XMLHttpRequest'
        }
    })
