                    ...
                    <div class="flex-lg-row-fluid me-xl-18 mb-10 mb-xl-0">
                        ...
                        <div class="mt-n1">
                            ...
                            <div class="d-flex flex-stack pb-10">
                                ...
                                <!--end::Logo-->
                                <!--begin::Action-->
                                <a href="https://www.quickteller.com/gombestatecollections" class="btn btn-sm btn-success" >QUICKTELLER</a>
                                
                                <a href="#" class="btn btn-sm btn-success" onclick="rrrPay()">Pay With Remita</a>
                                <a href="#" class="btn btn-sm btn-success" onclick="initiatePayment()">Pay Now</a>
                                <!--end::Action-->
                            </div>
                            <!--end::Top-->
                            <!--begin::Wrapper-->
                            <div class="m-0" id="viewInvoice">
                                ...
                                <div class="fw-bold fs-3 text-gray-800 mb-8" id="viewrefId">{{ $invoice->id }}</div>
                                 ...
                                 <div class="fw-bold fs-3 text-gray-800 mb-8"   id="viewInvoiceId">{{ $invoice->transaction_ref }}</div>
...
<br />
<!--<button  class="btn btn-sm btn-primary" onclick="posverify({{ $invoice->id }})">Mobile Print</button>-->
<button id="mobilePrintButton" class="btn btn-sm btn-primary" onclick="posverify({{ $invoice->id }})">Mobile Print</button>
<!--end::Action-->
                            </div>
                                <!--end::Table-->
...
@if(auth()->user()->hasRole("administrator"))
 ...
 <a> <span class="badge badge-light-warning" onclick="updatePaymentStatus({{ $invoice->id }})mobile print </span></a>
                                 </a> 
          <a id="verifyPaymentAndUpdateStatus" href="#" data-rrr="{{ $invoice->rate }}" data-invoice-id="{{ $invoice->invoice_id }}">
    ...
    <span class="badge badge-light-warning">Verify RRR Payment</span>
                       ...
                       <a id="downloadReceiptLink" href="#">
    <span class="badge badge-light-warning">Verify Card Payment</span>
    <a> <span class="badge badge-light-warning" onclick="updatePaymentStatus({{ $invoice->id }})"> via </span></a>
</a>
          @endif
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Add click event listener to verifyPaymentAndUpdateStatus button
    const verifyButton = document.getElementById("verifyPaymentAndUpdateStatus");
    if (verifyButton) {
        ...
        console.error("verifyPaymentAndUpdateStatus button not found");
    }

    // Fetch payment status and hide buttons if necessary
    const invoiceIdElement = document.getElementById("viewrefId");
    if (invoiceIdElement) {
        const invoiceId = invoiceIdElement.innerText.trim();
        // console.log("Fetching payment status for Invoice ID:", invoiceId);

        fetch(`/get-payment-statuss/${invoiceId}`)
            .then(response => response.json())
            .then(data => {
                // console.log("Payment status fetched:", data);
                //  console.log("Invoice ID: ", data.status);
                if (data.status === 'paid') {
                console.log("Invoice Status: ", data.status);
            
                // Hide the buttons if the payment status is 'paid'
                const downloadReceiptLink = document.getElementById("downloadReceiptLink");
                const verifyPaymentAndUpdateStatusButton = document.getElementById("verifyPaymentAndUpdateStatus");
                const mobilePrintButton = document.getElementById("mobilePrintButton");
            
                if (downloadReceiptLink) {
                    console.log("Hiding downloadReceiptLink");
                    downloadReceiptLink.style.display = 'none';
                } else {
                    console.error("downloadReceiptLink button not found");
                }
            
                if (verifyPaymentAndUpdateStatusButton) {
                    console.log("Hiding verifyPaymentAndUpdateStatusButton");
                    verifyPaymentAndUpdateStatusButton.style.display = 'none';
                } else {
                    console.error("verifyPaymentAndUpdateStatusButton button not found");
                }
            
                if (mobilePrintButton) {
                    console.log("Hiding mobilePrintButton");
                    mobilePrintButton.style.display = 'none';
                } else {
                    console.error("mobilePrintButton not found");
                }
            }
        ...
        console.error("Invoice ID element not found for fetching payment status");
    }
});
...
// POS receipt
function posverify(invoiceId) {
    fetch(`/posrct/${invoiceId}`, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
            "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').getAttribute('content'), // Add CSRF token
